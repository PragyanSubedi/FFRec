...
while True:
    ...
    for(x,y,w,h) in faces:
        ...
        Idu, conf1 = recognizer.predict(gray[y:y + h, x:x + w])
        for (ex, ey, ew, eh) in eyes:
            Id, conf2 = recognizer.predict(gray[ey:ey + eh, ex:ex + ew])
        ...
        cv2.putText(img,str(conf1), (x, y + h + 120 ), fontFace, fontScale, fontColor1)
        cv2.putText(img, str(conf2), (x, y + h + 150), fontFace, fontScale, fontColor1)
        if (conf1<80 and conf2<80):
